...
from networks import state_network, dynamics_network, policy_value_network_mu, policy_value_network_alpha
...
BT_K = 5
HISTORY_LENGTH = 2
INPUT_SHAPE = (HISTORY_LENGTH, BT_K, BT_K, 3)
ACTION_SHAPE = (BT_K, BT_K, 3)
...
if os.path.exists(training_data_path+"/input_data.npy"):
    ...
    states_count, replay_buffer_index = np.load(
        training_data_path+"status.npy")
...
class BufferThread(Thread):
    ...
    def run(self, limit=None):
        ...
        while self.continuer and ((limit is None) or (replay_buffer_index < limit)):
            ...
            input_data[idx] = np.array(
                new_input_data, dtype=float).reshape(INPUT_SHAPE)
            policy[idx] = np.array(
                new_policy, dtype=float).reshape(ACTION_SHAPE)
            value[idx] = new_value
            idx += 1
            states_count += 1
            replay_buffer_index = min(
                replay_buffer_index+1, REPLAY_BUFFER_SIZE)
            ...
            if idx % SAVE_REPLAY_BUFFER_FREQ == 0:
                ...
                np.save(training_data_path+"status",
                        (states_count, replay_buffer_index))
...
class ZerolGenerator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        y = {"policy": self.policy[begin_idx:end_idx],
...
if os.path.exists(model_path):
    ...
    network.compile(optimizer="adam", loss={
                    "policy": "categorical_crossentropy", "value": "mse"})
...
class StatsLogger(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.n > EVALUATION_FREQ and False:
            ...
            perf_against_random = int(subprocess.run(
                ["zerol_evaluate", "-p", "rand", "--only-result"], stdout=PIPE, stderr=DEVNULL).stdout.splitlines()[0])
            perf_against_flatmc = int(subprocess.run(
                ["zerol_evaluate", "-p", "flat", "--only-result"], stdout=PIPE, stderr=DEVNULL).stdout.splitlines()[0])
            #perf_against_flat_uct  = subprocess.run(["zerol_evaluate", "-p", "flat_ucb", "--only-result"]).stdout
            perf_against_uct = int(subprocess.run(
                ["zerol_evaluate", "-p", "uct", "--only-result"], stdout=PIPE, stderr=DEVNULL).stdout.splitlines()[0])
            #perf_against_rave    = subprocess.run(["zerol_evaluate", "-p", "rave", "--only-result"]).stdout
            print("VS RANDOM: {} | VS FLAT: {} | VS UCT: {}".format(
                perf_against_random, perf_against_flatmc, perf_against_uct))
            tf.summary.scalar(
                'vs_random', data=perf_against_random, step=epoch)
            tf.summary.scalar(
                'vs_flatmc', data=perf_against_flatmc, step=epoch)
