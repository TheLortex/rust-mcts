...
*/
pub struct PUCT<'a, G: Game, F: Fn(&G) -> (HashMap<G::Move, f32>, f32)> {
    pub C_PUCT: f32,
    pub evaluate: &'a F,
    pub _g: PhantomData<G>
}

impl<G: Game, F: Fn(&G) -> (HashMap<G::Move, f32>, f32)> Copy for PUCT<'_, G, F> {}

impl<G: Game, F: Fn(&G) -> (HashMap<G::Move, f32>, f32)> Clone for PUCT<'_,G, F> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<'a, G: Game, F: Fn(&G) -> (HashMap<G::Move, f32>, f32)> PolicyBuilder<G> for PUCT<'a, G,F> {
