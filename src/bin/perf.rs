...
async fn run() {
    ...
    let breakthrough: WithHistory<Breakthrough, _> =
        game_builder.create(Breakthrough::players()[0]);

    let board_size = breakthrough.state_dimension().size();
    let action_size = Breakthrough::action_dimension().size();
